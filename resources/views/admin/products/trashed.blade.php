...
@section('title', 'Archived Products - REGASCO SIS')
...
<div class="mb-6">
    <div class="flex items-center justify-between">
        <div>
            <a href="{{ route('admin.products.index') }}" class="text-gray-600 hover:text-gray-800 font-medium flex items-center">
                <i class="fas fa-arrow-left mr-2"></i> Back to Products
            </a>
            <p class="text-gray-500 text-sm mt-1">View and restore archived products</p>
        </div>
    </div>
</div>
        ...
        <table class="w-full">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-6 py-4 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Sales</th>
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Deleted</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($products as $product)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $product->deleted_at->format('M d, Y') }}</td>
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center space-x-2">
                                <button onclick="openRestoreModal({{ $product->product_id }}, '{{ $product->product_name }}')" 
                                    class="p-2 text-green-500 hover:text-green-700 transition-colors" title="Restore">
                                    <i class="fas fa-undo-alt"></i>
                                </button>
                                
                                <button onclick="openForceDeleteModal({{ $product->product_id }}, '{{ $product->product_name }}')" 
                                    class="p-2 text-red-500 hover:text-red-700 transition-colors" title="Permanently Delete">
                ...
                @empty
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                            ...
                            <p class="text-lg font-medium">No archived products</p>
                            <p class="text-sm text-gray-400">Deleted products will appear here</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full transform scale-95 opacity-0 transition-all duration-300" id="restoreModalContent">
        <div class="bg-gradient-to-r from-green-500 to-green-600 px-6 py-4 rounded-t-2xl">
            <div class="flex items-center space-x-3">
                ...
                <h3 class="text-white font-bold text-lg">Restore Product</h3>
            </div>
        </div>
        ...
        <div class="p-6">
            <div class="flex items-start space-x-4 mb-4">
                ...
                <div>
                    <p class="text-gray-800 font-medium mb-1">Are you sure you want to restore this product?</p>
                    ...
                    <p class="text-sm text-green-600 mt-2 font-medium">
                        ...
                        This product will be moved back to the active products list.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="px-6 py-4 bg-gray-50 rounded-b-2xl flex justify-end space-x-3">
            ...
            <form id="restoreForm" method="POST" class="inline">
                @csrf
                <!-- FIX: Changed from POST to PATCH to match route -->
                @method('PATCH')
                <button type="submit" 
                    ...
                    <i class="fas fa-undo-alt mr-2"></i>Restore Product
                </button>
            </form>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full transform scale-95 opacity-0 transition-all duration-300" id="forceDeleteModalContent">
        <div class="bg-gradient-to-r from-red-500 to-red-600 px-6 py-4 rounded-t-2xl">
            <div class="flex items-center space-x-3">
                ...
                <h3 class="text-white font-bold text-lg">Permanently Delete Product</h3>
            </div>
        </div>
        ...
        <div class="p-6">
            <div class="flex items-start space-x-4 mb-4">
                ...
                <div>
                    <p class="text-gray-800 font-medium mb-1">Are you sure you want to permanently delete this product?</p>
                    ...
                    <p class="text-sm text-red-500 mt-2 font-medium">
                        ...
                        Sales history will be preserved but unlinked. This action cannot be undone.
                    </p>
                </div>
            </div>
        </div>
...
<script>
    function openRestoreModal(productId, productName) {
        ...
        form.action = '{{ route("admin.products.restore", ":id") }}'.replace(':id', productId);
    ...
    function openForceDeleteModal(productId, productName) {
        ...
        form.action = '{{ route("admin.products.force-delete", ":id") }}'.replace(':id', productId);
